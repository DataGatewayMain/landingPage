...
let subscriptionForm = this.formBuilder.group({
  email: ['', [Validators.required, Validators.email]]
});

function submit() {
  if (subscriptionForm.valid) {
      const email = subscriptionForm.value.email;
      subscriptionForm.reset();
      this.snackbar.open('Thank you for subscribing', 'close', { duration: 3 * 1000 });
  } else if (subscriptionForm.get('email')?.hasError('required')) {
      this.snackbar.open('Email is required', 'close', { duration: 3 * 1000 });
  } else {
      // Handle invalid email format
      this.snackbar.open('Please provide a valid email format', 'close', { duration: 3 * 1000 });
  }
}
